...
<h4>Groups &raquo; <?php echo $group_rec['group_name'];?> &raquo; Sub Group List:</h4><br/>
...
$sql="SELECT group_id,group_name,group_type,group_level FROM groups WHERE group_owner=$current_logged_member AND parent_groupid=".$group_rec['group_id']." AND group_type=1 ORDER BY ABS(group_type) ASC,group_name ASC";
...
$link=($arr['group_level']<=6)?'<a href="groups_dashboard.php?grp='.$arr['group_id'].'">View</a>':'-';

$tr.='<tr><td>'.$i.'</td><td>'.$arr['group_name'].'</td><td>'.$CDYN_ARR['group_type'][$arr['group_type']].'</td><td>'.$link.'</td></tr>';
$i++;
}
...
if($tr!='') { echo '<table border="0" cellpadding="5" cellspacing="5"><tr><th>SNO</th><th>Group Name</th><th>Group Type</th><th>View</th></tr>'.$tr.'</table>'; }
	...
	echo "No groups to show.  <a href='groups_create_group.php'><strong>Add Group</strong></a>";
}
?>
